...
(function(module, exports) {
  (function(f, e) {
    ...
    f.Panel = function(e, f2, l) {
      var c = Infinity, k = 0, g = Math.round, a = g(window.devicePixelRatio || 1), r = 80 * a, h2 = 48 * a, t = 3 * a, v = 2 * a, d = 3 * a, m = 15 * a, n = 74 * a, p2 = 30 * a, q = document.createElement("canvas");
      q.width = r;
      q.height = h2;
      ...
      return { dom: q, update: function(h3, w) {
        ...
        b.drawImage(q, d + a, m, n - a, p2, d, m, n - a, p2);
        b.fillRect(d + n - a, m, a, p2);
        b.fillStyle = l;
        b.globalAlpha = 0.9;
        b.fillRect(d + n - a, m, a, g((1 - h3 / w) * p2));
      } };
    };
    return f;
  });
...
window.addEventListener("load", (event) => {
  if ("serviceWorker" in navigator) {
    ...
    navigator.serviceWorker.addEventListener("message", (event2) => {
      if (navigator.serviceWorker.controller !== null && "command" in event2.data && event2.data.command === "caches") {
        navigator.serviceWorker.controller.postMessage({ command: "clear", caches: event2.data.caches });
      }
    });
  }
  const app = createApp({
    ...
    watch: {
      ...
      tickerWidth(newValue) {
        ...
        anime({
          ...
          easing: "linear",
          update: () => {
            self2.animatedTickerWidth = obj.width;
          }
        });
      },
      backgroundImages: {
            ...
            if (elements.length > 1) {
              ...
              for (const element of elements) {
                ...
                const animation = element.animate(keyframes, {
                  fill: "forwards",
                  easing: "steps(" + offset + ")",
                  duration: 1e3 / frameRate * elements.length,
                  iterations: Infinity
                });
                if (startTime === null) {
    ...
    methods: {
      signIn: function(event2) {
        if (event2 === firebase.auth.GoogleAuthProvider.PROVIDER_ID) {
          firebase.auth().signInWithPopup(new firebase.auth.GoogleAuthProvider()).then((result) => {
            var credential = result.credential;
            for (const data2 of result.user.providerData) {
            ...
            database.ref(`${databaseRoot}/users/${result.user.uid}`).transaction(function(current) {
              if (current) {
                ...
                current = { name: data.displayName, timestamp };
              }
              return current;
            });
            try {
        ...
        } else if (event2 === firebase.auth.FacebookAuthProvider.PROVIDER_ID) {
        ...
        } else if (event2 === firebase.auth.TwitterAuthProvider.PROVIDER_ID) {
          firebase.auth().signInWithPopup(new firebase.auth.TwitterAuthProvider()).then((result) => {
            var credential = result.credential;
            const timestamp2 = Math.floor(new Date() / 1e3);
            for (const data2 of result.user.providerData) {
              const photoUrl = data2.photoURL.replace(/_normal\.jpg$/, ".jpg");
              firebase.auth().currentUser.updateProfile({
                displayName: data2.displayName,
                photoURL: photoUrl
              }).catch(function(error) {
              ...
              database.ref(`${databaseRoot}/users/${result.user.uid}`).transaction(function(current) {
                if (current) {
                  current["name"] = data2.displayName;
                  current["link"] = `https://twitter.com/${result.additionalUserInfo.username}`;
                  current["timestamp"] = timestamp2;
                } else {
                  current = { name: data2.displayName, link: `https://twitter.com/${result.additionalUserInfo.username}`, timestamp: timestamp2 };
                }
                return current;
              });
              break;
      ...
      update: async function(ignore = false) {
        ...
        if (ignore) {
          Object.keys(this.cachedTracks).forEach(function(key) {
            for (const handlerId of self2.cachedTracks[key].handlers) {
              Microsoft.Maps.Events.removeHandler(handlerId);
            }
            self2.map.entities.remove(self2.cachedTracks[key].pushpin);
            delete self2.cachedTracks[key];
          });
        }
        if (tracks !== null) {
          const timestamp2 = Math.floor(new Date() / 1e3);
          ...
          for (const geohash in tracks) {
            for (const track2 of tracks[geohash]) {
              let pushpinId = null;
              for (const id in this.cachedTracks) {
                if (track2.id === this.cachedTracks[id].id) {
                  pushpinId = id;
                  break;
                }
              }
              if (pushpinId === null) {
                const pushpin = new Microsoft.Maps.Pushpin(new Microsoft.Maps.Location(track2.location.latitude, track2.location.longitude), {
                  title: track2.name,
                  subTitle: this.formatTime(timestamp2 - track2.timestamp),
                  icon: "/images/Marker-Star.svg"
                });
                this.setImage(pushpin, track2.user);
                this.cachedTracks[pushpin.id] = track2;
                ...
                this.cachedTracks[pushpin.id]["handlers"] = [Microsoft.Maps.Events.addHandler(pushpin, "click", async (args) => {
                  self2.mode = this.cachedTracks[args.target.id];
                  self2.isRevealed = true;
                }), Microsoft.Maps.Events.addHandler(pushpin, "dblclick", (args) => {
                  window.location.hash = self2.cachedTracks[args.target.id].id;
                })];
                this.map.entities.push(pushpin);
                isUpdated = true;
              } else if (track2.timestamp > this.cachedTracks[pushpinId].timestamp) {
                const pushpin = this.cachedTracks[pushpinId]["pushpin"];
                const handlers = this.cachedTracks[pushpinId]["handlers"];
                pushpin.setLocation(new Microsoft.Maps.Location(track2.location.latitude, track2.location.longitude));
                pushpin.setOptions({
                  title: track2.name,
                  subTitle: this.formatTime(timestamp2 - track2.timestamp)
                });
                this.setImage(pushpin, track2.user);
                this.cachedTracks[pushpinId] = track2;
          ...
          Object.keys(this.cachedTracks).forEach(function(key) {
            if (!ids.some((x) => x === self2.cachedTracks[key].id)) {
              for (const handlerId of self2.cachedTracks[key].handlers) {
                Microsoft.Maps.Events.removeHandler(handlerId);
              }
              self2.map.entities.remove(self2.cachedTracks[key].pushpin);
          ...
          if (isUpdated) {
            ...
            const leaderboard = await new Promise((resolve2) => {
              const trackDictionary = {};
              const trackRanking = [];
              for (const key in tracks) {
                for (const track2 of tracks[key]) {
                  if (track2.user.id in trackDictionary) {
                    trackDictionary[track2.user.id].count++;
                  } else {
                    const t = Object.assign({}, track2.user);
                    t["count"] = 1;
                    trackDictionary[track2.user.id] = t;
                  }
                }
              }
              ...
              resolve2(trackRanking);
            });
            this.leaderboard.splice(0);
            for (const data2 of leaderboard) {
              this.leaderboard.push(data2);
            }
            try {
              const results = await new Promise((resolve2) => {
                ...
                for (const key in tracks) {
                  ...
                  let termSet = [];
                  for (const track2 of tracks[key]) {
                    tokens.push(track2.name);
                    if (!termSet.includes(track2.name)) {
                      if (track2.name in inverseDocumentFrequency) {
                        inverseDocumentFrequency[track2.name] += 1;
                      } else {
                        inverseDocumentFrequency[track2.name] = 1;
                      }
                      termSet.push(track2.name);
                    }
                  }
                ...
                resolve2([documents, scores]);
              });
              this.cachedDocuments.splice(0);
              this.tags.splice(0);
              for (const document2 of results[0]) {
                this.cachedDocuments.push(document2);
              }
      ...
      fetch: async function(ignore, latitude, longitude) {
        ...
        for (const geohash of geohashes) {
          if (!ignore && geohash in this.queryCache && timestamp2 - this.queryCache[geohash].timestamp < timeout) {
            for (const user of this.queryCache[geohash].data) {
              tempRecent.push(user);
              if (geohash in data2) {
                data2[geohash].push(user);
              } else {
                data2[geohash] = [user];
              }
            }
          ...
          tempCache[geohash] = { timestamp: timestamp2, data: [] };
          if (snapshot.exists()) {
            ...
            for (const key in dictionary) {
              ...
              if (geohash in data2) {
                data2[geohash].push(dictionary[key]);
              } else {
                data2[geohash] = [dictionary[key]];
              }
            }
          }
        }
        ...
        Object.keys(this.queryCache).forEach(function(key) {
          if (timestamp2 - self2.queryCache[key].timestamp >= timeout) {
            delete self2.queryCache[key];
          }
        });
        const recent = this.take(tempRecent.sort((x, y) => y.timestamp - x.timestamp), 100);
        Object.keys(data2).forEach(function(key) {
          for (let i = data2[key].length - 1; i > 0; i--) {
            if (!recent.includes(data2[key][i])) {
              data2[key].splice(i, 1);
            }
          }
      ...
      startPedometer: async function() {
        ...
        this.deviceMotion = (event2) => {
          if (event2.accelerationIncludingGravity) {
            const ag = event2.accelerationIncludingGravity;
            ...
            if (self2.isStepping) {
              if (d < 9.8) {
                ...
                if (self2.steps % 10 === 0) {
                  let _random2 = function(min, max) {
                    min = Math.ceil(min);
                    max = Math.floor(max);
                    return Math.floor(Math.random() * (max - min)) + min;
                  }, format2 = function(format3) {
                    var args = arguments;
                    return format3.replace(/\{(\d)\}/g, function(m, c) {
                      return args[parseInt(c) + 1];
                    });
                  };
                  var _random = _random2, format = format2;
                  const map = [];
                  let i = 0;
                  let minCount = Number.MAX_SAFE_INTEGER;
                  for (const group of self2.chars) {
                    let j = 0;
                    for (const d2 of group) {
                      map.push({ path: { row: i, column: j }, data: d2 });
                      j++;
                      if (d2.count < minCount) {
                        minCount = d2.count;
                      }
                    }
                    i++;
                  ...
                  const indexPath = map[_random2(0, map.length)].path;
                  const sequence = [];
                  self2.chars[indexPath.row][indexPath.column].count++;
                  self2.chars[indexPath.row][indexPath.column].timestamp = Math.floor(new Date() / 1e3);
                  for (const obj of this.prepare(this.character.sequences.filter((x) => x.name === "Capture"), self2.chars[indexPath.row][indexPath.column].set[0], this.character.sequences)) {
                    if (obj.type === "Message") {
                      sequence.push({ type: obj.type, speed: obj.speed, duration: obj.duration, character: this.character, text: format2(obj.text, self2.chars[indexPath.row][indexPath.column].set[0]) });
                    } else {
        ...
        if ("permissions" in navigator) {
          ...
          if (permissionStatus.state == "granted" || permissionStatus.state == "prompt") {
            ...
            navigator.geolocation.getCurrentPosition((position) => {
              self2.isLocating = false;
              self2.map.setView({
                center: new Microsoft.Maps.Location(position.coords.latitude, position.coords.longitude),
                zoom: self2.map.getZoom() < 16 ? 16 : self2.map.getZoom()
              });
            }, (error) => {
              self2.isLocating = false;
              self2.notify({ text: error.message, accent: self2.character.accent, image: self2.character.image });
              console.error(error);
            }, {
          ...
          if (this.input.length > 0) {
            ...
            if (keys.length > 0 && tags.length > 0) {
              ...
              if (this.isEditing) {
                ...
                const column = [];
                column.push({ set: [this.input], index: 0, count: 0, timestamp: Math.floor(new Date() / 1e3), reserved: false });
                if (typeof result === "undefined") {
                  this.chars.splice(0, 0, column);
                } else {
                  result.push({ set: [this.input], index: 0, count: 0, timestamp: Math.floor(new Date() / 1e3), reserved: false });
                }
                this.input = "";
      ...
      setImage: async function(pushpin, user, selected = false) {
        if ("image" in user) {
          let image;
          try {
            image = await new Promise(async (resolve2, reject) => {
              const i = new Image();
              i.onload = () => {
                resolve2(i);
              };
      ...
      upload: async function(event2) {
        ...
        const files = [];
        const paths = [];
        for (const file of event2.target.files) {
          files.push(file);
        }
          ...
          try {
            await new Promise(function(resolve2, reject) {
              uploadTask.on("state_changed", function(snapshot) {
                self2.progress = snapshot.bytesTransferred / snapshot.totalBytes / files.length + paths.length / files.length;
              }, function(error) {
                reject(error);
              }, function() {
                resolve2();
        ...
        database.ref(databaseRoot + "/images").push({ paths, timestamp: Math.floor(new Date() / 1e3) });
        this.progress = null;
      ...
      learn: async function(word) {
        function format(format2) {
          var args = arguments;
          return format2.replace(/\{(\d)\}/g, function(m, c) {
            return args[parseInt(c) + 1];
          });
        }
        const sequence = [];
        ...
        this.word = { name: word.name, attributes };
        if ("location" in word) {
      ...
      check: function(event2) {
        for (const attribute of this.word.attributes) {
          if (attribute === event2.target.dataset.attribute) {
            attribute.value = event2.target.checked;
          }
        }
      },
      share: async function(word) {
        ...
        database.ref(databaseRoot + "/users/" + this.user.uid + "/dictionary/words/" + word.name).transaction(function(current) {
          if (current) {
              ...
              for (const attribute of word.attributes) {
                if (attribute.value) {
                  if (attribute.name in current.attributes && current.attributes[attribute.name] > 0) {
                    ...
                    c.attributes[attribute.name] = timestamp2 - 1;
                  }
            ...
            current = { attributes: {}, timestamp: timestamp2 };
            for (const attribute of word.attributes) {
              if (attribute.value) {
                current.attributes[attribute.name] = timestamp2;
              } else {
          ...
          if (committed) {
            if (snapshot.exists()) {
              ...
              for (const key in dictionary.attributes) {
                if (typeof dictionary.attributes[key] === "number" && dictionary.attributes[key] > 0 && self2.attributes.includes(key)) {
                  timestamps.push(dictionary.attributes[key]);
                }
              }
              if (timestamps.length === 1 && timestamps[0] === dictionary.timestamp) {
                let format2 = function(format3) {
                  var args = arguments;
                  return format3.replace(/\{(\d)\}/g, function(m, c) {
                    return args[parseInt(c) + 1];
                  });
                ...
                var format = format2;
                database.ref(databaseRoot + "/users/" + self2.user.uid + "/dictionary/count").transaction(function(count) {
                  return (count || 0) + 1;
                });
                for (const obj of self2.prepare(self2.character.sequences.filter((x) => x.name === "Learned"))) {
                  if (obj.type === "Message") {
                    self2.notify({ text: format2(obj.text, word.name), accent: self2.character.accent, image: self2.character.image });
                  }
                }
                self2.isStared = true;
                window.setTimeout(() => {
                  self2.isStared = false;
                }, 3e3);
                if (!self2.isMuted) {
                  self2.$refs.twinkle.play();
                }
              }
              if ("user" in word === false || word.user.id === self2.user.uid) {
                database.ref(databaseRoot + "/tracks/" + await self2.digestMessage(`${self2.user.uid}&${word.name}`)).transaction(function(current) {
                  const attributes = {};
                  if (current) {
                    current["key"] = `${geohash}${timestamp2}`;
                    current["location"] = { latitude: location.latitude, longitude: location.longitude };
                    current["geohash"] = geohash;
                    current["timestamp"] = timestamp2;
                  } else {
                    current = { key: `${geohash}${timestamp2}`, name: word.name, location: { latitude: location.latitude, longitude: location.longitude }, geohash, user, timestamp: timestamp2 };
                  }
                  for (const key in dictionary.attributes) {
                    if (self2.attributes.includes(key)) {
                  ...
                  if (c) {
                    ...
                    self2.notify({ text: e.message, accent: self2.character.accent, image: self2.character.image });
                    console.error(e);
                  }
                  self2.isSubmitting = false;
                });
              } else {
                self2.isSubmitting = false;
              }
            ...
            console.error(error);
            self2.isSubmitting = false;
          } else if ("user" in word === false || word.user.id === self2.user.uid) {
            database.ref(databaseRoot + "/tracks/" + await self2.digestMessage(`${self2.user.uid}&${word.name}`)).transaction(function(current) {
              if (current) {
                current["key"] = `${geohash}${timestamp2}`;
                current["location"] = { latitude: location.latitude, longitude: location.longitude };
                ...
                for (const attribute of word.attributes) {
                  if (attribute.value) {
                    current.attributes[attribute.name] = timestamp2 - 1;
                  } else {
                    current.attributes[attribute.name] = 0;
                  }
                }
              } else {
                current = { key: `${geohash}${timestamp2}`, name: word.name, location: { latitude: location.latitude, longitude: location.longitude }, geohash, attributes: {}, user, timestamp: timestamp2 };
                for (const attribute of word.attributes) {
                  if (attribute.value) {
                    current.attributes[attribute.name] = timestamp2;
                  } else {
        ...
        if ("words" in this.mode) {
          ...
          if (snapshot.exists()) {
            ...
            for (const name in words) {
              tempWords.push("user" in words[name] ? { name, attributes: words[name].attributes, user: words[name].user } : { name, attributes: words[name].attributes });
            }
      ...
      previous: async function(userId, startAt, limit = 50) {
        ...
        if ("words" in this.mode && snapshot.exists()) {
          ...
          this.mode.words = [];
          for (const name in words) {
            this.mode.words.push("user" in words[name] ? { name, attributes: words[name].attributes, user: words[name].user } : { name, attributes: words[name].attributes });
          }
        }
      },
      discover: async function() {
        ...
        for (const word of shuffle(words)) {
          try {
            const s = await new Promise((resolve2, reject) => {
              database.ref(databaseRoot + "/users/" + self2.user.uid + "/dictionary/words/" + word.name).transaction(function(current) {
                if (current) {
                  return void 0;
                }
                return current;
            ...
            if (s !== null) {
              ...
              for (const obj of this.prepare(this.character.alternative.sequences.filter((x) => x.name === "Discover"), word.name, this.character.alternative.sequences)) {
                if (obj.type === "Message") {
                  sequence.push({ type: obj.type, speed: obj.speed, duration: obj.duration, character: this.character.alternative, text: format2(obj.text, word.name) });
                } else {
      ...
      activate: async function() {
        ...
        if (this.cachedDocuments.length > 0) {
          if (this.documentQueue.length == 0) {
            ...
            var shuffle = shuffle2;
            for (const document3 of shuffle2(this.cachedDocuments)) {
              this.documentQueue.push(document3);
            }
          }
          const document2 = this.documentQueue.shift();
          if (!await this.talk(this.user.uid, document2.filter((x) => x !== this.character.name))) {
      ...
      talk: async function(userId, tokens = []) {
        ...
        if (tokens.length > 0) {
          ...
          for (const token of tokens) {
            if (token in this.wordDictionary === false || timestamp2 - this.wordDictionary[token].timestamp >= timeout) {
              const snapshot = await database.ref(databaseRoot + "/users/" + userId + "/dictionary/words/" + token).once("value");
              this.wordDictionary[token] = { attributes: [], timestamp: timestamp2 };
              if (snapshot.exists()) {
          ...
          for (const s of shuffle2(sequences)) {
                    ...
                    for (const obj of preparedSequence) {
                      if (obj.type == "Message") {
                        ...
                        if (temp === null) {
                          ...
                          let cache2;
                          [text, cache2] = temp;
                          sequence.push({ type: obj.type, speed: obj.speed, duration: obj.duration, text });
                        }
                      } else {
                        sequence.push(obj);
                      }
      ...
      generate: async function(userId, message, hints = []) {
        ...
        const epsilon = Math.pow(10, -6);
        const beamWidth = 10;
        let sequences = [{ sequence: [], score: 1 }];
          ...
          if (!tokenSet.includes(token)) {
            if (Array.isArray(token)) {
              ...
              for (const attribute of token) {
                if (attribute in hintDictionary) {
                  for (const s2 of hintDictionary[attribute]) {
                    if (!terms.includes(s2)) {
                      let isNew = true;
                      terms.push(s2);
                  ...
                  if (attribute in this.reverseWordDictionary === false || timestamp2 - this.reverseWordDictionary[attribute].timestamp >= timeout) {
                    ...
                    this.reverseWordDictionary[attribute] = { words: [], timestamp: timestamp2 };
                    if (snapshot.exists()) {
                      const words = snapshot.val();
              ...
              if (terms.length > 0 && scores.length > 0) {
                ...
                for (let i = 0; i < sequences.length; i++) {
                  for (let j = 0; j < probabilities.length; j++) {
                    ...
                    sequence.push({ index, term: terms[j] });
                    candidates.push({ sequence, score: sequences[i].score * probabilities[j] });
                  }
                }
                sequences.splice(0);
      ...
      notify: function(data2, duration = 3e3) {
        const self2 = this;
        data2["id"] = window.setTimeout((d) => {
          for (let i = 0; i < self2.notifications.length; i++) {
            if (self2.notifications[i].id === d.id) {
              self2.notifications.splice(i, 1);
              break;
            }
          }
        }, duration, data2);
        this.notifications.unshift(data2);
      },
      blinded: async function() {
        if (this.backgroundImagesQueue.length == 0) {
          ...
          var shuffle = shuffle2;
          for (const image2 of shuffle2(this.recentImages)) {
            this.backgroundImagesQueue.push(image2);
          }
        }
      ...
      formatDate: function(event2) {
        moment.locale(window.navigator.language);
        return moment(event2).format("LT");
      },
      ...
      animationEnd: function(el) {
        const self2 = this;
        this.$nextTick(() => {
          self2.notificationHeight = self2.$refs.notifications.getBoundingClientRect().height;
          self2.leaderboardHeight = self2.$refs.leaderboard.getBoundingClientRect().height;
        });
        if (!this.isPopup) {
          this.message = null;
        }
      ...
      tickerUpdated: function(el) {
        ...
        this.$nextTick(() => {
          for (const clip of document.body.querySelectorAll("#input>.columns:last-of-type>.column>.control .clip")) {
            ...
            if (width > 0) {
              self2.tickerWidth = Math.min(width / 2, document.body.querySelector("#input>.columns:last-of-type>.column>.control .level").getBoundingClientRect().width);
              clip.querySelector(":scope .ticker-wrap .ticker").style.width = width + "px";
            }
          }
        });
      ...
      range: function(date, days) {
        ...
        for (const day of days) {
          collection.push(new Date(new Date(date.getFullYear(), date.getMonth(), date.getDate(), date.getHours(), date.getMinutes(), date.getSeconds(), date.getMilliseconds()).getTime() + day * 24 * 60 * 60 * 1e3));
        }
        return collection;
      },
                  ...
                  if (sequenceStack.length > 0) {
                    ...
                    do {
                      ...
                      if (sequenceStack.length > 0) {
                        ...
                        for (const o of sequenceStack[sequenceStack.length - 1].sequence) {
                          if (o.type == "Sequence") {
                            if ("sequence" in o) {
                              let isNew = true;
                              for (const tempSequence of stack2) {
                                if (tempSequence === o) {
                                  isNew = false;
                                  break;
                                }
                      ...
                      if ("sequence" in poppedSequence) {
                        ...
                        if (isNew) {
                          stack2.push(poppedSequence);
                        }
                      }
                    } while (sequenceStack.length > 0);
                    while (stack2.length > 0) {
                      tracedSequences.push(stack2.pop());
                    }
                  } else {
        ...
        if (sequenceStack[sequenceStack.length - 1] !== targetSequence) {
          if ("sequence" in sourceSequence) {
            for (const o of sourceSequence.sequence) {
              if (o.type == "Sequence") {
                let stack2 = this.getSequenceStack(o, targetSequence);
                if (stack2.length > 0 && stack2[stack2.length - 1] === targetSequence) {
                  let tempStack = [];
                  do {
      ...
      animate: async function(timestamp2) {
        ...
        if (this.character !== null) {
          ...
          var _random = _random2;
          const deltaTime = (timestamp2 - this.elapsed) / 1e3;
          this.elapsed = timestamp2;
            ...
            if (this.sequenceQueue.length == 0) {
              if (activateTime >= activateThreshold) {
                if (this.cachedDocuments.length > 0) {
                  ...
                  const document2 = this.documentQueue.shift();
                  this.talk(this.user.uid, document2.filter((x) => x !== this.character.name));
                }
                idleTime = activateTime = 0;
          ...
          if (!this.isLocked && this.sequenceQueue.length > 0) {
            ...
            if (sequence.length > 0) {
              ...
              Object.keys(this.cachedImages).forEach(function(key) {
                if (!self2.cachedSprites.some((x) => x.source === key)) {
                  delete self2.cachedImages[key];
                }
              });
          ...
          if (this.message !== null) {
            if (this.message.type.reverse) {
              if (this.message.type.count > 0) {
                ...
                if (this.message.type.elapsed >= 1 / this.message.type.speed) {
              ...
              if (charArray.length > 0) {
                for (let i = 0; i < this.message.type.count; i++) {
                  if (this.message.text.charAt(i) == "\n") {
                    ...
                    randomBuffer += charArray[~~_random2(0, charArray.length)];
                  }
                }
              }
              ...
              for (const a of animations) {
                for (const sprite of a.images) {
                  if (sprite.source in this.cachedImages === false) {
                    try {
                      const image = await new Promise(async (resolve2, reject) => {
                        const i = new Image();
                        i.onload = () => {
                          resolve2(i);
                        };
    ...
    mounted: async function() {
      ...
      let credential = null;
      let fragments;
      const characters = [{ path: "/assets/milch.json", probability: 1 }];
      const alternatives = [{ path: "/assets/merku.json", probability: 1 }];
      if (botStorageItem) {
        try {
      ...
      if (statsStorageItem) {
        const baseDate = new Date().getTime() - 7 * 24 * 60 * 60 * 1e3;
        try {
          for (const day of JSON.parse(statsStorageItem)) {
            const date = new Date(day.date);
            if (date.getTime() > baseDate) {
              this.stats.push({ date, steps: day.steps });
            }
          }
        } catch (e) {
          localStorage.removeItem("stats");
      ...
      try {
        ...
        if (response1.ok) {
          ...
          if (response3.ok) {
            const timestamp2 = Math.floor(new Date() / 1e3);
            const baseTime = new Date().getTime() - 24 * 60 * 60 * 1e3;
            let index = 0;
            for (const group of await response3.json()) {
              const g = [];
              for (const set2 of group) {
                const result = fragments.find((x) => set2.some((y) => x.set.includes(y)) && x.count > 0 && x.timestamp * 1e3 > baseTime && x.checksum === [...String(x.timestamp)].reduce((x2, y) => x2 + y, 0) + [...String(x.count)].reduce((x2, y) => x2 + y, 0));
                if (typeof result === "undefined") {
                  g.push({ set: set2, index: 0, count: 0, timestamp: timestamp2, reserved: true });
                } else {
                  g.push({ set: set2, index: 0, count: result.count, timestamp: result.timestamp, reserved: true });
                }
              }
              this.chars.push(g);
            }
          ...
          database.ref(databaseRoot + "/tracks").orderByChild("timestamp").limitToLast(10).on("value", (snapshot) => {
            if (snapshot.exists()) {
              ...
              for (const word of tempWords.sort((x, y) => y.timestamp - x.timestamp)) {
                let removeIndex = -1;
                for (let i = 0; i < self2.recentWords.length; i++) {
                  if (self2.recentWords[i].name == word.name) {
                    removeIndex = i;
                    break;
                  }
                }
                if (removeIndex >= 0) {
                  self2.recentWords.splice(removeIndex, 1);
                }
                self2.recentWords.splice(index, 0, word);
                index++;
              }
              for (let i = self2.recentWords.length - 1; i >= index; i--) {
                self2.recentWords.splice(i, 1);
              }
            }
          });
        } else if (self2.user !== null) {
          database.ref(databaseRoot + "/users/" + self2.user.uid + "/dictionary/count").off("value");
  ...
  window.addEventListener("dblclick", (event2) => {
  });
  ...
  window.addEventListener("touchend", (event2) => {
    event2.stopPropagation();
  });
  window.addEventListener("touchcancel", (event2) => {
    event2.stopPropagation();
  });
