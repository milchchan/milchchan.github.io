...
export class Agent {
    ...
    async load(url) {
      ...
      for (const url of ["images/Milch-ClosedMouth.svg", "images/Milch-Half-ClosedEyes-ClosedMouth.svg", "images/Milch-ClosedEyes-ClosedMouth.svg"]) {
        ...
        if (!(image instanceof Event) || image.type !== "error") {
          this.cachedSprites[url] = { image: image, width: this.character.width * this.scale, height: this.character.height * this.scale, delay: 0.1 };
        } else {
          console.error(image);
        }
      }
    ...
    run() {
      ...
      this.animationQueue.push(Object.assign({ time: 0 }, this.cachedSprites["images/Milch-ClosedMouth.svg"]));
        ...
        if (timestamp > self.previousTime) {
          ...
          if (self.balloonCanvas.style.visibility !== "visible") {
            ...
            if (self.animationQueue.length === 0) {
              self.animationQueue.push(Object.assign({ time: 0 }, self.cachedSprites["images/Milch-ClosedMouth.svg"]));
              self.animationQueue.push(Object.assign({ time: 0 }, self.cachedSprites["images/Milch-Half-ClosedEyes-ClosedMouth.svg"]));
              self.animationQueue.push(Object.assign({ time: 0 }, self.cachedSprites["images/Milch-ClosedEyes-ClosedMouth.svg"]));
              self.animationQueue.push(Object.assign({ time: 0 }, self.cachedSprites["images/Milch-Half-ClosedEyes-ClosedMouth.svg"]));
              self.animationQueue.push(Object.assign({ time: 0 }, self.cachedSprites["images/Milch-ClosedMouth.svg"]));
            }
          }
    ...
    renderCharacter(deltaTime) {
      if (this.animationQueue.length > 0) {
        let frame = this.animationQueue[0];
        ...
        while (frame.time >= delay) {
          ...
          frame.time = 0;
          frame = this.animationQueue.shift();
        ...
        if (frame.image !== null) {
          ...
          backContext.drawImage(frame.image, 0, 0, frame.width * window.devicePixelRatio, frame.height * window.devicePixelRatio);
