...
def main(req: func.HttpRequest) -> func.HttpResponse:
    try:
        ...
        if req.method == 'PUT':
            ...
            container = database.get_container_client('Posts')
            item = list(container.query_items(
                query="SELECT p.id, p.slug, p.type, p.input, p.message, p.animations, p.nsfw, p.random, p.accesses, p.timestamp FROM Posts AS p WHERE p.id LIKE CONCAT(@identifier, '%') ORDER BY p.timestamp ASC OFFSET 0 LIMIT 1",
                parameters=[
                    {'name': '@identifier', 'value': identifier}
                ],
                enable_cross_partition_query=True))[0]
