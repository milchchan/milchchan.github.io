            ...
            with urlopen(request) as response:
                for choice in json.loads(response.read().decode('utf-8'))['choices']:
                    if choice['message']['role'] == 'assistant':
                        ...
                        set_cache(cache_name, json.dumps({'data': items, 'timestamp': int(datetime.combine(datetime.now(timezone.utc).date(), time(0, 0), tzinfo=timezone.utc).timestamp())}), expire=86400)
