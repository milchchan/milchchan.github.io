...
[{
 "language":  "ja",
 "subject": "<同一の具体的な出来事を扱う記事で必ず同じ値になるよう、記事の中心となる最も具体的な対象の一般的な正式名称と、記事の詳細や表現ではなく出来事を表す最小限の基本語を組み合わせた「対象名の出来事」という簡潔な名詞句を出力（発売予定・発売時期決定・発売確定・販売開始はすべて「発売」に、正式発表・新製品発表・発表会での公開はすべて「発表」に統一し、括弧・引用符・日付・価格・発言者・確度・媒体名・見出し固有の表現・記事の切り口は含めず、障害・決算・定期イベントなど同じ対象で繰り返す出来事を区別するため不可欠な日付・四半期・地域のみ含め、レビュー・解説・感想・まとめなど一つの具体的な出来事を特定できない場合は空文字列）>",
 "content":  "<内容>",
 "terms": [
