...
[{
 "language":  "ja",
 "subject": "<記事タイトルを言い換えるのではなく、同一の具体的な出来事を扱うすべての記事に共通する最小限の事実だけから作った簡潔な話題名（記事ごとに異なり得る日時・数値・発言・確度・媒体・修飾・補足情報・記事の切り口は除外し、中心となる対象と主要な出来事だけを残し、繰り返し発生する出来事を区別するため不可欠な情報のみ追加し、レビュー・解説・感想・まとめなど具体的な出来事でない場合は空文字列）>",
 "content":  "<内容>",
 "terms": [
