...
# 出力形式
出力形式は以下のJSONフォーマットとします。このフォーマット以外で出力しないでください。重要度の高い内容、面白い内容、興味深い内容、ユーモアな内容、高い技術の内容を、高く評価しスコアをつけてください。まとめやスポンサーのがついているものや宣伝や販促・キャンペーン系のような内容はスコアを低くつけてください。配列の長さが10件を超える場合は、日付が新しい順に並び替え上位10件にしてください。
```json
[{
    "language":  "ja",
    "content":  "<内容>",
