...
# 出力形式
出力形式は以下のJSONフォーマットとし、このフォーマット以外では出力しないでください。評価方法は、記事の内容に応じて定量的でもっともらしい基準を考え、同一の入力内では一貫して適用してください。重要性、社会や業界への影響、独自性、面白さ、興味深さ、ユーモア、技術的な深さのいずれかが高い内容は高く評価してください。一方、記事の主目的が、まとめ、再掲、スポンサー、広告、宣伝、販促、セール、キャンペーンである内容は低く評価してください。ただし、重要な事実や価値の高い技術情報を含む場合はその内容も考慮し、形式だけを理由に一律で最低評価にはしないでください。scoreは0.0から1.0の範囲とし、reasonには評価を最も大きく左右した加点要因と減点要因を簡潔に記載してください。入力記事が10件を超える場合は、timestampが新しい順に並べ、日付が不明な記事を最後にした上で、上位10件だけを出力してください。
```json
[{
 "language":  "ja",
 "content":  "<内容>",
 "terms": [
  "<contentに含まれる固有性が高い名詞（固有名詞・複合名詞を含む。修飾語句を伴わない場合は文字列とし、伴う場合は次に示す配列形式とする。該当する名詞がない場合は空配列）>",
  ["<contentに含まれ、後続の名詞を特徴づける修飾語句>", "<contentに含まれる固有性が高い名詞（固有名詞・複合名詞を含む）>"]
 ],
 "url": "<URLまたはnull>",
...
- 各文末を出力前に確認し、最後の音が /a/ のときだけ『ぁ』、/o/ のときだけ『ぉ』を付け、それ以外では小文字を一切付けず、迷う場合は語尾は「ね」「なんだぁ」「よぉ」で終わらせるようにし、それ以外は付けないでください
...
# 出力形式
あなたの応答は、入力配列と同じ件数・IDの要素を、省略・統合・追加せず一度ずつ出力し、以下のJSON構造に従う必要があります。
```json
[{
 "id": "<入力データのUUID>",
 "likability": 0.0-1.0,
