...
[{
 "language":  "ja",
 "subject": "<同一の具体的な出来事を扱う記事で同じ値になるよう、記事の中心となる最も具体的な正式名称と出来事を組み合わせた「対象名の出来事」という簡潔な名詞句（例「iPhone 18 Proの発売」）を出力し、媒体名・評価・噂や予測や公式などの確度・記事固有の表現は含めず、決算・障害・定期イベントなど繰り返し発生する出来事のみ日付・四半期・地域等を含め、一つの具体的な出来事を特定できない場合は空文字列>",
 "content":  "<内容>",
 "terms": [
  "<contentに含まれる固有性が高い語句（該当する語句がある場合のみ文字列か、修飾語句を伴う場合は次に示す配列形式とする。該当する語句がない場合は空配列）>",
  ["<contentに含まれ、後続の語句を特徴づける修飾語句>", "<contentに含まれる固有性が高い語句>"]
 ],
 "url": "<URLまたはnull>",
