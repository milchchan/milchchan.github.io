...
[{
 "language":  "ja",
 "subject": "<記事タイトルの言い換えではなく、同じ対象と出来事を扱う記事で同じ文字列になることを最優先した正規化済みの簡潔な話題名（中心となる対象には記事中で確認できる最も具体的な正式名称を使い、一般名・上位ブランド名・略称への言い換えや、鉤括弧・引用符・敬称・媒体名を避ける。主要な出来事は意味を変えない最短の一般的な名詞形で表し、記事ごとに異なり得る日時・数値・発言・確度・評価・修飾・補足情報・記事の切り口は除外する。ただし、繰り返し発生する出来事を区別するため不可欠な情報は残す。具体的な出来事でない記事は記事全体の中心テーマを同じ規則で表し、必ず空でない文字列にする）>",
 "content":  "<内容>",
 "terms": [
