    ...
    try:
        if req.method == 'POST':
                ...
                if 'status' in data:
                    ...
                    if image is None:
                        ...
                        if response.getcode() == 200:
                            ...
                            user = status['user']
                            item = {'id': status['id'], 'text': status['text'], 'timestamp': int(datetime.strptime(status['created_at'], '%a %b %d %H:%M:%S +0000 %Y').timestamp()), 'user': {
                                'id': user["screen_name"], 'name': user['name'], 'image': user['profile_image_url_https']}}
                        ...
                        if match:
                            ...
                            if mime_type in ['image/gif', 'image/png', 'image/jpeg'] and encoding == 'base64':
                                ...
                                boundary = md5(
                                    str(random.randint(0, 32000)).encode()).hexdigest()
                                data = f'--{boundary}\r\n'.encode()
                                ...
                                if response.getcode() == 200:
                                    media_id = json.loads(response.read())[
                                        'media_id_string']
            ...
            if response.getcode() == 200:
                ...
                return func.HttpResponse(json.dumps(data), status_code=200, headers={'Access-Control-Allow-Origin': req.headers['Origin']} if 'Origin' in req.headers else {}, mimetype='application/json', charset='')
        ...
        return func.HttpResponse(status_code=405, headers={'Allow': 'GET, POST, OPTIONS'}, mimetype='', charset='')
