    ...
    try:
        if 'X-Authorization' in req.headers:
            ...
            if match:
                ...
                path = req.params['path'] if 'path' in req.params else None
                ...
                if path is None:
                    if geohash is None:
                        if from_date is None:
                            items = list(container.query_items(
                                query=f'SELECT l.id, l.data, l.timestamp FROM Logs AS l WHERE l.timestamp <= @to_date ORDER BY l.timestamp {"DESC" if order == "desc" else "ASC"} OFFSET @offset LIMIT @limit',
                                parameters=[
                        ...
                        if from_date is None:
                            ...
                            items = list(container.query_items(
                                query=f"SELECT l.id, l.data, l.timestamp FROM Logs AS l WHERE l.timestamp > @from_date AND l.timestamp <= @to_date AND (l.geohash LIKE CONCAT(@centergeohash, '%') OR l.geohash LIKE CONCAT(@topgeohash, '%') OR l.geohash LIKE CONCAT(@bottomgeohash, '%') OR l.geohash LIKE CONCAT(@rightgeohash, '%') OR l.geohash LIKE CONCAT(@leftgeohash, '%') OR l.geohash LIKE CONCAT(@topleftgeohash, '%') OR l.geohash LIKE CONCAT(@toprightgeohash, '%') OR l.geohash LIKE CONCAT(@bottomrightgeohash, '%') OR l.geohash LIKE CONCAT(@bottomleftgeohash, '%')) ORDER BY l.timestamp {'DESC' if order == 'desc' else 'ASC'} OFFSET @offset LIMIT @limit",
                                parameters=[
                                    {'name': '@offset', 'value': 0 if offset is None else offset},
                                    {'name': '@limit', 'value': 100 if limit is None else limit},
                                    {'name': '@from_date', 'value': datetime.fromtimestamp(from_date, timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')},
                    ...
                    if from_date is None:
                        ...
                        datetime.fromtimestamp(time.time(), timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')
                        items = list(container.query_items(
                            query=f'SELECT l.id, l.data, l.timestamp FROM Logs AS l WHERE l.timestamp > @from_date AND l.timestamp <= @to_date ORDER BY l.timestamp {"DESC" if order == "desc" else "ASC"} OFFSET @offset LIMIT @limit',
                            parameters=[
                                {'name': '@offset', 'value': 0 if offset is None else offset},
                                {'name': '@limit', 'value': 100 if limit is None else limit},
                                {'name': '@from_date', 'value': datetime.fromtimestamp(from_date, timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')},
                    ...
                    if from_date is None:
                        items = list(container.query_items(
                            query=f"SELECT l.id, l.data, l.timestamp FROM Logs AS l WHERE l.path = @path AND l.timestamp <= @to_date AND (l.geohash LIKE CONCAT(@centergeohash, '%') OR l.geohash LIKE CONCAT(@topgeohash, '%') OR l.geohash LIKE CONCAT(@bottomgeohash, '%') OR l.geohash LIKE CONCAT(@rightgeohash, '%') OR l.geohash LIKE CONCAT(@leftgeohash, '%') OR l.geohash LIKE CONCAT(@topleftgeohash, '%') OR l.geohash LIKE CONCAT(@toprightgeohash, '%') OR l.geohash LIKE CONCAT(@bottomrightgeohash, '%') OR l.geohash LIKE CONCAT(@bottomleftgeohash, '%')) ORDER BY l.timestamp {'DESC' if order == 'desc' else 'ASC'} OFFSET @offset LIMIT @limit",
                            parameters=[
                                {'name': '@offset', 'value': 0 if offset is None else offset},
                                {'name': '@limit', 'value': 100 if limit is None else limit},
                        ...
                        items = list(container.query_items(
                            query=f"SELECT l.id, l.data, l.timestamp FROM Logs AS l WHERE l.path = @path AND l.timestamp > @from_date AND l.timestamp <= @to_date AND (l.geohash LIKE CONCAT(@centergeohash, '%') OR l.geohash LIKE CONCAT(@topgeohash, '%') OR l.geohash LIKE CONCAT(@bottomgeohash, '%') OR l.geohash LIKE CONCAT(@rightgeohash, '%') OR l.geohash LIKE CONCAT(@leftgeohash, '%') OR l.geohash LIKE CONCAT(@topleftgeohash, '%') OR l.geohash LIKE CONCAT(@toprightgeohash, '%') OR l.geohash LIKE CONCAT(@bottomrightgeohash, '%') OR l.geohash LIKE CONCAT(@bottomleftgeohash, '%')) ORDER BY l.timestamp {'DESC' if order == 'desc' else 'ASC'} OFFSET @offset LIMIT @limit",
                            parameters=[
                                {'name': '@offset', 'value': 0 if offset is None else offset},
                                {'name': '@limit', 'value': 100 if limit is None else limit},
                                {'name': '@path', 'value': path},
