    ...
    try:
        ...
        if from_date is None:
            if mime_type is None:
                ...
                items = list(container.query_items(
                    query=f'SELECT p.id, p.type, p.message, p.animations, p.nsfw, p.views, p.timestamp FROM Posts AS p WHERE p.type LIKE @mime_type AND p.timestamp <= @to_date ORDER BY p.timestamp {"DESC" if order == "desc" else "ASC"} OFFSET @offset LIMIT @limit' if nsfw else f'SELECT p.id, p.type, p.animations, p.nsfw, p.views, p.timestamp FROM Posts AS p WHERE p.type LIKE @mime_type AND NOT p.nsfw AND p.timestamp <= @to_date ORDER BY p.timestamp {"DESC" if order == "desc" else "ASC"} OFFSET @offset LIMIT @limit',
                    parameters=[
                        {'name': '@offset', 'value': 0 if offset is None else offset},
                        {'name': '@limit', 'value': 100 if limit is None else limit},
            ...
            if mime_type is None:
                ...
                items = list(container.query_items(
                    query=f'SELECT p.id, p.type, p.message, p.animations, p.nsfw, p.views, p.timestamp FROM Posts AS p WHERE p.type LIKE @mime_type AND p.timestamp > @from_date AND p.timestamp <= @to_date ORDER BY p.timestamp {"DESC" if order == "desc" else "ASC"} OFFSET @offset LIMIT @limit' if nsfw else f'SELECT p.id, p.type, p.animations, p.nsfw, p.views, p.timestamp FROM Posts AS p WHERE p.type LIKE @mime_type AND NOT p.nsfw AND p.timestamp > @from_date AND p.timestamp <= @to_date ORDER BY p.timestamp {"DESC" if order == "desc" else "ASC"} OFFSET @offset LIMIT @limit',
                    parameters=[
                        {'name': '@offset', 'value': 0 if offset is None else offset},
                        {'name': '@limit', 'value': 100 if limit is None else limit},
                        {'name': '@mime_type', 'value': mime_type},
