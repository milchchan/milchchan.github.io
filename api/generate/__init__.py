            ...
            if content_type == 'application/json':
                ...
                if 'boundary=' in content_type:
                    ...
                    if audio_data is not None and json_data is not None:
                        ...
                        with urlopen(Request(api_url + '/upload', data=data, headers={'User-Agent': user_agent, 'Content-Type': f'multipart/form-data; boundary={boundary}'}, method='POST')) as response:
                        ...
                        with urlopen(Request(api_url + '/queue/join', data=json.dumps({
                            ...
                            'fn_index': 0,
                            'session_hash': session
                        }).encode(), headers={'User-Agent': user_agent, 'Content-Type': 'application/json'} if bearer_token is None else {'Authorization': f"Bearer {bearer_token}", 'User-Agent': user_agent, 'Content-Type': 'application/json'})) as response:
                            event_id = json.loads(response.read().decode('utf-8'))['event_id']
                        ...
                        if path is None:
                            ...
                            if not cache_exists and reference_text is None:
                                identifier = str(uuid4())
