            ...
            if content_type == 'application/json':
                ...
                if llm_source is None or len(llm_source) == 0:
                    ...
                    if api_key is None:
                        ...
                        request = Request(f'https://generativelanguage.googleapis.com/v1beta/{os.environ["GEMINI_API_MODEL_CODE"]}:generateContent?key={api_key}', data=json.dumps({'contents': contents} if temperature is None else {'contents': contents, 'generationConfig': {'temperature': temperature}}).encode('utf-8'), method='POST', headers={'Content-Type': 'application/json'})
