                ...
                if llm_source is None or len(llm_source) == 0:
                    ...
                    if 'X-Authorization' in req.headers:
                        ...
                        if api_key is None or len(api_key) == 0:
                            ...
                            with urlopen(Request('https://api.openai.com/v1/responses', data=json.dumps({'model': data['model'] if 'model' in data else os.environ['OPENAI_MODEL'], 'input': messages, 'temperature': data['temperature'] if 'temperature' in data else 1.0, 'tools': data['tools'] if 'tools' in data else []}).encode('utf-8'), method='POST', headers={'Authorization': f'Bearer {api_key}', 'Content-Type': 'application/json'})) as response:
