                ...
                if llm_source is None or len(llm_source) == 0:
                    ...
                    if 'X-Authorization' in req.headers:
                        ...
                        if api_key is None or len(api_key) == 0:
                            ...
                            with urlopen(Request('https://api.openai.com/v1/responses', data=json.dumps({'model': model, 'input': messages, 'temperature': 1.0 if model.startswith('gpt-5') else data['temperature'] if 'temperature' in data else 1.0, 'reasoning': data['reasoning'] if 'reasoning' in data else {'effort': 'minimal'} , 'tools': data['tools'] if 'tools' in data else [{'type': 'mcp', 'server_label': 'milchchan-mcp', 'server_url': 'https://milchchan.com/api/mcp', 'require_approval': 'never'}]}).encode('utf-8'), method='POST', headers={'Authorization': f'Bearer {api_key}', 'Content-Type': 'application/json'})) as response:
